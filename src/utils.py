...
#Return sparse pairwise matrix
def parameterize_seq_pairwise(seq,pair_wt,seq_dict):
    #now create a pairwise parameterized seq
    pair = []
    ls = len(seq_dict)
    for i in range(len(seq)-1):
        for q in range(i + 1,len(seq)):
            pair.append(ls*seq_dict[seq[i]] + seq_dict[seq[q]])
    #remove wt pairs
    param_list = []
    counter = 0
    for i,p in enumerate(pair):
        if p < pair_wt[i]:
            param_list.append(counter*(ls**2-1) + pair[i])
        elif p > pair_wt[i]:
            param_list.append(counter*(ls**2-1) + pair[i]-1)
        counter=counter+1
    return param_list

def make_pair_matrix(df,wt,dicttype):
    seq_dict,inv_dict = choose_dict(dicttype)
    ls = len(seq_dict)
    wt_pairs = []
    for i in range(len(wt)-1):
        for q in range(i+1,len(wt)):
            wt_pairs.append(ls*seq_dict[wt[i]] + seq_dict[wt[q]])
    matrix = sp.sparse.lil_matrix((len(df.index),(sp.misc.comb(len(wt),2)*(len(seq_dict)**2-1))))
    entries_list = df['seq'].apply(parameterize_seq_pairwise,args=(wt_pairs,seq_dict))
    for i,entry in enumerate(list(entries_list)):
        matrix[i,entry] = 1
    return sp.sparse.csr_matrix(matrix)

def parameterize_pair_emat(matrix,wt,seq_dict):
    wt_pairs = [len(seq_dict)*seq_dict[wt[i]] + seq_dict[wt[i+1]] for i in range(len(wt)-1)]
    new_matrix = np.zeros((len(seq_dict)**2-1,len(wt)-1))
    for c in range(len(wt)-1):        
        matrix[:,c] = matrix[:,c] - matrix[wt_pairs[c],c]
        new_matrix[:,c] = np.delete(matrix[:,c],wt_pairs[c])
    out_matrix = new_matrix.ravel(order='F')
    return out_matrix
   
def inverse_pair_parameterization(matrix,wt,seq_dict):
    '''takes in dense raveled matrix, reshapes and then fixes so its not based
    around the wild type anymore'''
    wt_pairs = [len(seq_dict)*seq_dict[wt[i]] + seq_dict[wt[i+1]] for i in range(len(wt)-1)] 
    matrix = matrix.reshape(len(seq_dict)**2-1,len(wt)-1,order='F')
    out_matrix = np.zeros((len(seq_dict)**2,len(wt)-1))
    #add in implicit wild type row at top
    #now insert zeros to set row 0 = 'AA', row 1 = 'AC' etc.
    for c in range(len(wt)-1):
        out_matrix[:,c] = np.insert(matrix[:,c],wt_pairs[c],0)
    return out_matrix

#Three point stuff!!!
#Return sparse pairwise matrix
def parameterize_seq_tp(seq,tp_wt,seq_dict):
    #now create a pairwise parameterized seq
    tp = []
    ls = len(seq_dict)
    for i in range(len(seq)-1):
        for q in range(i + 1,len(seq)-1):
            for z in range(q + 1, len(seq)):
                tp.append(ls**2*seq_dict[seq[i]] + ls*seq_dict[seq[q]] + seq_dict[seq[z]])
    #remove wt pairs
    param_list = []
    counter = 0
    for i,p in enumerate(tp):
        if p < tp_wt[i]:
            param_list.append(counter*(ls**3-1) + tp[i])
        elif p > tp_wt[i]:
            param_list.append(counter*(ls**3-1) + tp[i]-1)
        counter=counter+1
    return param_list

def make_tp_matrix(df,wt,dicttype):
    seq_dict,inv_dict = choose_dict(dicttype)
    ls = len(seq_dict)
    wt_tp = []
    for i in range(len(wt)-1):
        for q in range(i + 1,len(wt)-1):
            for z in range(q + 1, len(wt)):
                wt_tp.append(ls**2*seq_dict[wt[i]] + ls*seq_dict[wt[q]] + seq_dict[wt[z]])
    matrix = sp.sparse.lil_matrix((len(df.index),(sp.misc.comb(len(wt),3)*(len(seq_dict)**3-1))))
    entries_list = df['seq'].apply(parameterize_seq_tp,args=(wt_tp,seq_dict))
    for i,entry in enumerate(list(entries_list)):
        matrix[i,entry] = 1
    return sp.sparse.csr_matrix(matrix)


def parameterize_tp_emat(matrix,wt,seq_dict):
    wt_tp = [seq_dict[wt[i]]*len(seq_dict)**2 + seq_dict[wt[i+1]]*len(seq_dict) + seq_dict[wt[i+2]] for i in range(len(wt)-2)]
    new_matrix = np.zeros((len(seq_dict)**3-1,len(wt)-2))
    for c in range(len(wt)-2):        
        matrix[:,c] = matrix[:,c] - matrix[wt_tp[c],c]
        new_matrix[:,c] = np.delete(matrix[:,c],wt_tp[c])
    out_matrix = new_matrix.ravel(order='F')
    return out_matrix
   
def inverse_tp_parameterization(matrix,wt,seq_dict):
    '''takes in dense raveled matrix, reshapes and then fixes so its not based
    around the wild type anymore'''
    wt_tp = [seq_dict[wt[i]]*len(seq_dict)**2 + seq_dict[wt[i+1]]*len(seq_dict) + seq_dict[wt[i+2]] for i in range(len(wt)-2)] 
    matrix = matrix.reshape(len(seq_dict)**3-1,len(wt)-2,order='F')
    out_matrix = np.zeros((len(seq_dict)**3,len(wt)-2))
    #add in implicit wild type row at top
    #now insert zeros to set row 0 = 'AAA', row 1 = 'AAC' etc.
    for c in range(len(wt)-2):
        out_matrix[:,c] = np.insert(matrix[:,c],wt_tp[c],0)
    return out_matrix  
...
def genweightandmat(weights_df,seq_dict,means=None,modeltype='LinearEmat'):
    ...
    for i,s in enumerate(weights_df['seq']):
             ...
             if i ==0:
                 print [weights_df[binheaders[z]][i]*z for z in range(nbins)]/weights_df['ct'][i]   
...
#def genweightandmat_v2(weights_df,seq_dict,means=None,modeltype='LinearEmat'):
#    '''For use with learn_matrix, linear regressions. Generates a flattened
#         matrix representation of the sequences, with corresponding batch and 
#         number of counts (in sequence weighting vector)'''
#    
...
#         lasso_mat = sp.sparse.lil_matrix((n_seqs,len(seq_dict)*(mut_region_length-1)))
#    counter = 0
#    sample_weights = np.zeros(n_seqs*nbins)
#    batch = np.zeros(n_seqs*nbins)
#    
#    for i,s in enumerate(weights_df['seq']):
#             if modeltype == 'LinearEmat':
#                 lasso_mat[counter:counter+n_bins,:] = seq2matsparse(s,seq_dict)
#             elif modeltype == 'Neighbor':
#                 lasso_mat[counter:counter+n_bins,:] = seq2mat_sparse_neighbor(s,seq_dict)
#             else:
#                  raise ValueError('Incorrect Model Type')
#             '''Find mean batch number for each sequence. If means is supplied,
#                 instead find a weighted average.'''
...
#                 batch[i] = np.sum([weights_df[z][i]*means[z] for z in means.index]/(weights_df['ct'][i]))
#             else:  
#                 batch[counter:counter+n_bins] = 
#             sample_weights[counter:counter+n_bins] = weights_df['ct'][i]
#             
#    return sp.sparse.csr_matrix(lasso_mat),batch,sample_weights
